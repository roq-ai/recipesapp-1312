...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['recipe-creator', 'recipe-explorer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CommunityInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/communities` : null),
    () => getCommunities({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Communities',
      path: isTenantUser ? `/communities/view/${data?.[0]?.id}` : '/communities',
      entity: 'community',
    ...
    { name: 'Favorites', path: '/favorites', entity: 'favorite', service: AccessServiceEnum.PROJECT, icon: FiHeart },
    { name: 'Follows', path: '/follows', entity: 'follow', service: AccessServiceEnum.PROJECT, icon: FiUserPlus },
    { name: 'Recipes', path: '/recipes', entity: 'recipe', service: AccessServiceEnum.PROJECT, icon: FiClipboard },
