...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'recipe.create': {
    roles: ['recipe-explorer'],
    key: 'new-recipe-created',
    tenantPath: ['community', 'recipe'],
    userPath: [],
  },
  'comment.create': {
    roles: ['recipe-creator'],
    key: 'new-comment-on-recipe',
    tenantPath: ['community', 'recipe', 'comment'],
    userPath: [],
  },
  'follow.create': {
    roles: ['recipe-creator'],
    key: 'new-follower',
    tenantPath: ['community', 'user', 'follow'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['recipe-creator'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['recipe-creator', 'recipe-explorer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.community.findFirst({
