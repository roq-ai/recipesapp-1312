...
type ColumnType = ColumnDef<RecipeInterface, unknown>;
...
export function RecipeListPage(props: RecipeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRecipes({
        relations: ['user', 'community', 'comment.count', 'favorite.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['title.contains', 'ingredients.contains', 'instructions.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RecipeInterface>>(
    () => `/recipes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'instructions', header: 'Instructions', accessorKey: 'instructions' },
    { id: 'cooking_time', header: 'cooking_time', accessorKey: 'cooking_time' },
    ...
    hasAccess('community', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/communities/view/${record.community?.id}`}>
    ...
    hasAccess('comment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Comment',
          accessorKey: 'comment',
          cell: ({ row: { original: record } }: any) => record?._count?.comment,
        }
      : null,
    hasAccess('favorite', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.favorite,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/recipes/view/${record.id}`} passHref legacyBehavior>
                <Button
