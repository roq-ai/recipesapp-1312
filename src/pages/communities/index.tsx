...
import { getCommunities, deleteCommunityById } from 'apiSdk/communities';
import { CommunityInterface } from 'interfaces/community';

type ColumnType = ColumnDef<CommunityInterface, unknown>;
...
export function CommunityListPage(props: CommunityListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CommunityInterface>>(
    () => `/communities?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CommunityInterface) => {
    if (hasAccess('community', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/communities/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('recipe', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Recipe',
          accessorKey: 'recipe',
          cell: ({ row: { original: record } }: any) => record?._count?.recipe,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/communities/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('community', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/communities/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
