...
import { getFavorites, deleteFavoriteById } from 'apiSdk/favorites';
import { FavoriteInterface } from 'interfaces/favorite';

type ColumnType = ColumnDef<FavoriteInterface, unknown>;
...
export function FavoriteListPage(props: FavoriteListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FavoriteInterface>>(
  ...
  const handleView = (row: FavoriteInterface) => {
    if (hasAccess('favorite', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('recipe', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'recipe',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/recipes/view/${record.recipe?.id}`}>
              {record.recipe?.title}
            </Link>
          ),
        }
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="favorite" />
        </Flex>

        {hasAccess('favorite', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
