...
import { getFollows } from 'apiSdk/follows';
...
import { FavoriteListPage as ShowCaseList } from 'pages/favorites';

function DashboardPage() {
  const { session } = useSession();

  const followStat = useSWR<PaginatedInterface<FollowInterface>>(
    () => `/follows`,
    () => getFollows({ limit: 1, offset: 0 }),
  );

  const favoriteStat = useSWR<PaginatedInterface<FavoriteInterface>>(
    () => `/favorites`,
    () => getFavorites({ limit: 1, offset: 0 }),
  );

  const recipeStat = useSWR<PaginatedInterface<RecipeInterface>>(
    () => `/recipes`,
    () => getRecipes({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Follows',
      result: followStat?.data?.totalCount,
      isLoading: followStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Favorites',
      result: favoriteStat?.data?.totalCount,
      isLoading: favoriteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Recipes',
      result: recipeStat?.data?.totalCount,
      isLoading: recipeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
